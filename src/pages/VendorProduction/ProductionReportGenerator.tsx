...
function ProductionReportGenerator({ vendor, tasks }: Props) {
  ...
  const handleDownload = async () => {
    try {
      ...
      const templateProtection = await extractTemplateProtection(buffer, protectedSheetName);
      ...
      tasks.forEach((task, index) => {
        ...
        row.getCell(3).value = typeof task.projectCode === 'string' ? task.projectCode : ''; // Columna C
        row.getCell(4).value = typeof task.quantity === 'string' ? parseFloat(task.quantity) : 0; // Columna D
        row.getCell(6).value = typeof task.receivedDate === 'string' ? task.receivedDate : ''; // Columna F
        row.getCell(7).value = typeof task.completionDate === 'string' ? task.completionDate : ''; // Columna G
...
async function applyWorkbookProtection(
  ...
  try {
    ...
    const serializer = new XMLSerializer();
    zip.file(workbookPath, serializer.serializeToString(workbookDoc));
    zip.file(relationshipsPath, serializer.serializeToString(relationshipsDoc));
...
function applyWorksheetProtection(
  ...
  const namespace =
    ...
    'http://schemas.openxmlformats.org/spreadsheetml/2006/main';
  const protectionElement = existing ?? sheetDoc.createElementNS(namespace, 'sheetProtection');
...
async function extractTemplateProtection(
  ...
  try {
    ...
    const workbookProtectionElement = workbookDoc
      .getElementsByTagName('workbookProtection')
      .item(0);
    ...
    const sheetDoc = parser.parseFromString(sheetXml, 'application/xml');
    const sheetProtectionElement = sheetDoc.getElementsByTagName('sheetProtection').item(0);
